...
class Matchgame extends Component {
  state = {
    ...
    score: 0,
    imageUrlList: '',
    isActiveTabItem: tabsList[0].tabId,
  }
  ...
  getImage = () => {
    const lengthOfImageList = imagesList.length
    this.gettingRandom =
      imagesList[Math.floor(Math.random() * lengthOfImageList)]

    const {imageUrl} = this.gettingRandom

    this.setState({imageUrlList: imageUrl})
  }
  ...
  resetGame = () => {
    ...
    this.getImage()
    const image = this.gettingRandom.imageUrl
    this.setState({
      isGameRunning: true,
      timeRemaining: 60,
      score: 0,
  ...
  onChangeTabId = id => {
    this.setState({isActiveTabItem: id})
  }
  ...
  filteredThumbnailImages = () => {
    const {isActiveTabItem, imageUrlList} = this.state
    const filteredList = imagesList.filter(
      each => each.category === isActiveTabItem,
    )

    return (
      <div className="bottom-container">
        <div className="image-container">
          <img src={imageUrlList} alt="match" className="image" />
        </div>
        <ul className="list-container">
          {tabsList.map(eachItem => (
            <TabItem
              key={eachItem.tabId}
              tabDetails={eachItem}
              isActive={isActiveTabItem === eachItem.tabId}
        ...
        <ul className="filtered-list-container">
          {filteredList.map(eachList => (
            <Tablist
              ...
              onCheckingIdOfImage={this.onCheckingIdOfImage}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    const {isGameRunning, score, timeRemaining} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {isGameRunning ? this.filteredThumbnailImages() : this.finalScoreCard()}
      </div>
    )
  }
}
